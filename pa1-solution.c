...
static void process_instruction(int opcode, int *operands, int nr_operands)  
{
	...
	if (opcode == 0) { /* in R-format */
		...
		case 0x23:	/* LW */
			registers[rt] =
				( memory[registers[rs] + imm    ] << 24 ) +
				( memory[registers[rs] + imm + 1] << 16 ) +
				( memory[registers[rs] + imm + 2] <<  8 ) +
				( memory[registers[rs] + imm + 3]       );
			break;
		case 0x2b:	/* SW */
			memory[registers[rs] + imm    ] = (registers[rt] >> 24) & 0x000000ff;
			memory[registers[rs] + imm + 1] = (registers[rt] >> 16) & 0x000000ff;
			memory[registers[rs] + imm + 2] = (registers[rt] >>  8) & 0x000000ff;
			memory[registers[rs] + imm + 3] = (registers[rt]      ) & 0x000000ff;
			break;
		default:
...
{
	printf("*********************************************************\n");
	printf("*            >> ICT331 MIPS translator  v0.01 <<        *\n");
	printf("*                                                       *\n");
	printf("*                                       .--.            *\n");
	printf("*                           .--------.  |__|            *\n");
	printf("*                           |.------.|  |=.|            *\n");
